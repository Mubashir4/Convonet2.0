...
const Transcribe = () => {
  ...
  const recordingIntervalRef = useRef(null); // Interval for 30-second recordings

  useEffect(() => {
    const savedHistory = sessionStorage.getItem('transcriptionHistory');
    if (savedHistory) {
      const decryptedHistory = decryptData(savedHistory);
      console.log('Decrypted History:', decryptedHistory); // Debugging log
  ...
  const startRecording = () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      ...
      // Set an interval to stop and start recording every 30 seconds
  ...
  const removeRepetitions = (text) => {
    ...
    // Use a Set to keep track of unique words encountered
    const uniqueWords = new Set();

    // Split the text into words, preserving punctuation and whitespace
    const words = text.split(/\b\W+\b/); // Split on word boundaries and non-word characters

    // Build the output string with unique words
    let result = "";
    for (const word of words) {
      if (!uniqueWords.has(word.toLowerCase())) { // Check for case-insensitive duplicates
        uniqueWords.add(word.toLowerCase()); // Add the word (lowercase) to the Set
        result += word + " "; // Append the word with a space
      }
    }
    ...
    console.log("Processed Text:", result); // Log processed text
    return result;
  };
